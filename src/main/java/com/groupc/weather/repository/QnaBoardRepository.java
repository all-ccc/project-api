...
public interface QnaBoardRepository extends JpaRepository<QnaBoardEntity, Integer> {
    ...
    @Query(
        value = 
        "SELECT Q.board_number AS boardNumber, Q.title AS boardTitle,Q.write_datetime AS boardWriteDatetime, U.user_number AS boardWriterNumber, U.nickname AS boardWriterNickname, U.profile_image_url AS boardWriterProfileImageUrl, Q.reply_complete AS replyComplete FROM Qna_Board Q, User U WHERE Q.user_number = U.user_number AND (Q.title LIKE '%:searchword%' OR Q.content LIke'%:searchword%') GROUP BY Q.board_number ORDER BY Q.write_datetime DESC ", 
    nativeQuery = true
    )
    public List<QnaBoardListResultSet> findQnaBoardSearchList(@Param("searchword") String searchWord);
    ...
    public List<QnaBoardListResultSet2> getQnaBoardSearchList(@Param("searchword") String searchWord);
}
